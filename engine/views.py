...
def ctfLevelStats(request, level):
    ...
    for x in range(4, -1, -1):
        ...
        month_finds_count.append(
            FlagFind.objects.filter(flag__in=level_flags).filter(found_on__month=now.month - x).count())
...
def ctfStatsChart(request, level):
    ...
    if 'month' in request.POST:
        ...
        chart_title = "Flags Found Between " + str(start_obj.day) + "-" + month_names[start_obj.month - 1] + "-" + str(
            start_obj.year) + " and " + str(end_obj.day) + "-" + month_names[end_obj.month - 1] + "-" + str(
            end_obj.year)
        ...
        for i in range(delta.days + 1):
            ...
            finds_count.append(
                level_flag_finds.filter(found_on__day=tmp_date.day, found_on__month=tmp_date.month).count())
